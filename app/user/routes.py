...
def dashboard():
    ...
    total_usage = (
        db.session.query(db.func.sum(UsageRecord.data_used))
    ...
    usage_data = (
        db.session.query(
            UsageRecord.timestamp, db.func.sum(UsageRecord.data_used).label("total_data")
        )
        .filter(UsageRecord.user_id == current_user.id)
    ...
    recent_connections = (
        UsageRecord.query.filter_by(user_id=current_user.id)
...
def usage_history():
    ...
    usage_data = (
        db.session.query(
            UsageRecord.timestamp, UsageRecord.data_used, UsageRecord.connection_time
        )
        .filter(
